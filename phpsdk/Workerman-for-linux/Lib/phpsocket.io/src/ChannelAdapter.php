...
class ChannelAdapter extends DefaultAdapter
{
    ...
    public function __construct($nsp)
    {
        ...
        \Workerman\Lib\Channel\Client::connect(self::$ip, self::$port);
        \Workerman\Lib\Channel\Client::$onMessage = array($this, 'onChannelMessage');
        \Workerman\Lib\Channel\Client::subscribe("socket.io#/#");
    ...
    public function add($id ,$room)
    {
        ...
        \Workerman\Lib\Channel\Client::subscribe($channel);
    }
    ...
    public function del($id, $room)
    {
        ...
        if(!empty($this->rooms[$room]))
        {
            ...
            $channel = "socket.io#/#$room#";
            \Workerman\Lib\Channel\Client::unsubscribe($channel);
        }
    }
        ...
        if (!$remote) 
        {
            ...
            if(!empty($opts['rooms'])) 
            {
              foreach($opts['rooms'] as $room)
              {
                  ...
                  \Workerman\Lib\Channel\Client::publish($chn, $msg);
              }
            }
            else
            {
